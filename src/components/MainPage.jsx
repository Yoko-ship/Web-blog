            ...
            <div className="MainPage-div">
                <div>
                    <form>
                        ...
                        <button type="button" onClick={buttonHandler}>Отправить</button>
                    </form>
                </div>
            </div>
            <div className="grids">
                {elements.map((div) => (
                    <div className="grid" key={div.id}>
                        ...
                        {/* Delete button */}
                        <div className="btns">
                            <button onClick={()=> Delete(div.id)} className="delete-btn">Delete</button>
                                                            {/* EDIT BUTTON */}
                            {editMode === div.id ? (
                                <>
                                                                {/* EDIT BUTTON */}
                                <button className="save-btn" onClick={() => Edit(div.id)}>Save Name</button>
                                ...
                                <>
                                {/* EDIT BUTTON */}
                                <button className="edit-btn" onClick={() => startEditing(div.id,div.content.find(item => item.type === "h1").value)}>Edit Name</button>
                               
                                </>
                )}
                            {editModeGrade === div.id? (
                                <button className="save-btn" onClick={() => EditGrade(div.id)}>Save Mark</button>
                            ):(
                                <button className="edit-btn" onClick={() => startEditingGrade(div.id,div.content.find(item => item.type === "span").value)}>Edit Mark</button>
                            )}
                        </div>
                    </div>
                ))}
